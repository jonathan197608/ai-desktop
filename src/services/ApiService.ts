import { getOpenAIWebSearchParams } from '@/config/models'
import { SEARCH_SUMMARY_PROMPT } from '@/config/prompts'
import i18n from '@/i18n'
import store from '@/store'
import { setGenerating } from '@/store/runtime'
import { Assistant, MCPTool, Message, Model, Provider } from '@/types'
import { formatMessageError, isAbortError } from '@/utils/error'
import { withGenerateImage } from '@/utils/formats'
...
export async function fetchChatCompletion({
  ...
  EventEmitter.emit(EVENT_NAMES.RECEIVE_MESSAGE, message).then(() => onResponse(message))
...
export function checkApiProvider(provider: Provider): {
  ...
  if (provider.id !== 'ollama' && provider.id !== 'lmstudio') {
    if (!provider.apiKey) {
      window.message.error({ content: i18n.t('message.error.enter.api.key'), key, style }).then()
      return {
        valid: false,
  ...
  if (!provider.apiHost) {
    window.message.error({ content: i18n.t('message.error.enter.api.host'), key, style }).then()
    return {
      valid: false,
  ...
  if (isEmpty(provider.models)) {
    window.message.error({ content: i18n.t('message.error.enter.model'), key, style }).then()
    return {
      valid: false,
 ...
 */
export const formatApiKeys = (value: string): string => {
