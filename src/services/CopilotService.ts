...
class CopilotService {
  ...
  public getUser = async (token: string): Promise<UserResponse> => {
    try {
      ...
      const response = await (await fetch(CONFIG.API_URLS.GITHUB_USER, config)).json()
      return {
  ...
  public saveCopilotToken = async (token: string): Promise<void> => {
    try {
      await window.copilotStore.set('access_token', token)
      await window.copilotStore.save()
    } catch (error) {
      console.error('Failed to save access_token:', error)
  ...
  public getToken = async (
    ...
    try {
      this.updateHeaders(headers)
      const access_token = await window.copilotStore.get('access_token')
      const config = {
        headers: {
          ...this.headers,
      ...
      return await (await fetch(CONFIG.API_URLS.COPILOT_TOKEN, config)).json()
    } catch (error) {
      console.error('Failed to get Copilot access_token:', error)
  ...
  public logout = async (): Promise<void> => {
    try {
      try {
        await window.copilotStore.delete('access_token')
