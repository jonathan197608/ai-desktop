...
import {DEFAULT_CONTEXTCOUNT} from '@/config/constant'
import {getTopicById} from '@/hooks/useTopic'
import i18n from '@/i18n'
import store from '@/store'
import {Assistant, Message, Model, Topic} from '@/types'
import {getTitleFromString, uuid} from '@/utils'
...
export function getContextCount(assistant: Assistant, messages: Message[]) {
  ...
  const currentContextCount = clearIndex === -1 ? _messages.length : _messages.length - (clearIndex + 1);
