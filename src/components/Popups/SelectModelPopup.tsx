...
import { getModelLogo, isEmbeddingModel, isRerankModel } from '@/config/models'
import db from '@/databases'
import { useProviders } from '@/hooks/useProvider'
import { getModelUniqId } from '@/services/ModelService'
...
const PopupContainer: React.FC<PopupContainerProps> = ({ model, resolve }) => {
  ...
  useEffect(() => {
    const loadPinnedModels = async () => {
      ...
      const validPinnedModels = savedPinnedModels.filter((id: any) => allModelIds.includes(id))
  ...
  const getFilteredModels = useCallback(
    (provider: any) => {
      let models = provider.models.filter((m: any) => !isEmbeddingModel(m) && !isRerankModel(m))
      ...
      if (searchText.trim()) {
        ...
        // 如果不是搜索状态，过滤掉已固定的模型
        models = models.filter((m: any) => !pinnedModels.includes(getModelUniqId(m)))
      }
        ...
        label: (
          <ModelItem>
            ...
            <PinIcon
              onClick={(e) => {
                e.stopPropagation()
                togglePin(getModelUniqId(m)).then()
              }}
        ...
        label: (
          <ModelItem>
            ...
            <PinIcon
              onClick={(e) => {
                e.stopPropagation()
                togglePin(getModelUniqId(m.model)).then()
              }}
              isPinned={true}>
              <PushpinOutlined />
            </PinIcon>
