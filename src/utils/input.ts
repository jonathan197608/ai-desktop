...
export const getFilesFromDropEvent = async (e: React.DragEvent<HTMLDivElement>): Promise<FileType[]> => {
  if (e.dataTransfer.files.length > 0) {
    const results = await Promise.allSettled([...e.dataTransfer.files].map((file) => window.api.file.get(file.name)))
    const list: FileType[] = []
    for (const result of results) {
      if (result.status === 'fulfilled') {
        ...
        await Logger.error('[src/renderer/src/utils/input.ts] getFilesFromDropEvent:', result.reason)
      }
    }
    return list
  } else {
