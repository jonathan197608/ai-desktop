...
const GithubCopilotSettings: FC<GithubCopilotSettingsProps> = ({provider: initialProvider, setApiKey}) => {
  ...
  const handleGetDeviceCode = useCallback(async () => {
    try {
      ...
      message.open({content: t('settings.provider.copilot.code_failed'), type: 'error'})
    } finally {
      setLoading(false)
    }
  }, [t, defaultHeaders])
  ...
  const handleGetToken = useCallback(async () => {
    try {
      ...
      if (token) {
        ...
        setApiKey(token)
        message.open({content: t('settings.provider.copilot.auth_success'), type: 'success'})
      }
    } catch (error) {
      console.error('Failed to get token:', error)
      message.open({content: t('settings.provider.copilot.auth_failed'), type: 'error'})
    } finally {
      setLoading(false)
    }
  ...
  const handleLogout = useCallback(async () => {
    try {
      ...
      message.open({content: t('settings.provider.copilot.logout_success'), type: 'success'})
    } catch (error) {
      console.error('Failed to logout:', error)
      message.open({content: t('settings.provider.copilot.logout_failed'), type: 'error'})
      // 如果登出失败，重置登出状态
      updateProvider({...provider, apiKey: '', isAuthed: false})
      setApiKey('')
    } finally {
  ...
  const handleCopyUserCode = useCallback(async () => {
    ...
    message.open({content: t('common.copied'), type: 'success'})
  }, [userCode, t])

  // 打开验证页面
  const handleOpenVerificationPage = useCallback(() => {
    if (verificationUri) {
      openUrl(verificationUri).then(() => setVerificationPageOpened(true))
    }
  }, [verificationUri])
  ...
  const handleUpdateHeaders = useCallback(async () => {
    try {
      ...
      message.open({content: t('message.save.success.title'), type: 'success'})
    } catch (error) {
      message.open({content: t('settings.provider.copilot.invalid_json'), type: 'error'})
    }
