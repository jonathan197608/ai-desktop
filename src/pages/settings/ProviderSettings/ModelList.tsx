...
import { getModelLogo } from '@/config/models'
import { PROVIDER_CONFIG } from '@/config/providers'
import { useAssistants, useDefaultModel } from '@/hooks/useAssistant'
import { useProvider } from '@/hooks/useProvider'
import { ModelCheckStatus } from '@/services/HealthCheckService'
import { useAppDispatch } from '@/store'
import { setModel } from '@/store/assistants'
...
const ModelList: React.FC<ModelListProps> = ({ provider: _provider, modelStatuses = [], searchText = '' }) => {
  ...
  const sortedModelGroups = sortBy(toPairs(modelGroups), [0]).reduce((acc: Record<string, Model[]>, [key, value]) => {
    acc[key] = value
    return acc
  }, {})
    ...
    <>
      {Object.keys(sortedModelGroups).map((group) => (
        <Card
          ...
          style={{ marginBottom: '10px', border: '1px solid var(--color-border)' }}
          size="small">
