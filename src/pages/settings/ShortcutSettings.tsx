...
import { initialState, resetShortcuts, toggleShortcut, updateShortcut } from '@/store/shortcuts'
...
const ShortcutSettings: FC = () => {
  ...
  const handleKeyDown = async (e: React.KeyboardEvent, record: Shortcut) => {
    e.preventDefault()

    const keys: string[] = []
    if (e.ctrlKey) keys.push((window.isMac) ? 'Control' : 'Ctrl')
