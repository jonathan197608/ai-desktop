...
import { isSupportedResoningEffortModel } from '@/config/models'
import { codeThemes } from '@/context/SyntaxHighlighterProvider'
import { useAssistant } from '@/hooks/useAssistant'
import { useSettings } from '@/hooks/useSettings'
import { SettingDivider, SettingRow, SettingRowTitle, SettingSubtitle } from '@/pages/settings'
import AssistantSettingsPopup from '@/pages/settings/AssistantSettings'
...
import { Assistant, AssistantSettings, CodeStyleVarious, ThemeMode, TranslateLanguageVarious } from '@/types'
...
const SettingsTab: FC<Props> = (props) => {
  ...
  const onContextCountChange = (value: any) => {
    if (!isNaN(value as number)) {
      onUpdateAssistantSettings({contextCount: value})
    }
  }

  const onMaxTokensChange = (value: any) => {
    if (!isNaN(value as number)) {
      onUpdateAssistantSettings({maxTokens: value})
    }
  }

  const onReasoningEffortChange = (value: any) => {
        ...
        <SettingRow>
          ...
          <StyledSelect
            ...
            options={[
              ...
              {
                value: 'Command+Enter',
                label: `${window.isMac ? '⌘' : window.isWindows ? 'Win' : 'Super'} + Enter`
              }
            ]}
