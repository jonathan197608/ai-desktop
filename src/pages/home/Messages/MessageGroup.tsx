...
import { MultiModelMessageStyle } from '@/store/settings'
      ...
      {isGrouped && (
        <MessageGroupMenuBar
          ...
          setMultiModelMessageStyle={(style) => {
            setMultiModelMessageStyle(style)
            messages.forEach((message) => {
              editMessage(message.id, { multiModelMessageStyle: style }).then()
            })
          }}
          messages={messages}
  ...
  ${({ $layout, $isInPopover, $isGrouped }) => {
    return $layout === 'grid' && $isGrouped
      ? css`
          ...
          border: 1px solid ${$isInPopover ? 'transparent' : 'var(--color-border)'};
          padding: 10px;
          border-radius: 6px;
