...
import SelectModelPopup from '@/components/Popups/SelectModelPopup'
import TextEditPopup from '@/components/Popups/TextEditPopup'
import {isReasoningModel} from '@/config/models'
import {TranslateLanguageOptions} from '@/config/translate'
...
import {getMessageTitle, resetAssistantMessage} from '@/services/MessagesService'
...
import {withMessageThought} from '@/utils/formats'
...
const MessageMenubar: FC<Props> = (props) => {
  ...
  const onCopy = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (message.role === 'assistant' && message.model && isReasoningModel(message.model)) {
        ...
        navigator.clipboard.writeText(removeTrailingDoubleSpaces(processedMessage.content.trimStart())).then()
      } else {
        // 其他情况直接复制原始内容
        navigator.clipboard.writeText(removeTrailingDoubleSpaces(message.content.trimStart())).then()
      }

      window.message.success({content: t('message.copied'), key: 'copy-message'}).then()
      setCopied(true)
  ...
  const onEdit = useCallback(async () => {
    ...
    if (editedText && editedText !== textToEdit) {
      ...
      resendMessage && await handleResendUserMessage({...message, content: editedText})
    }
    ...
    () => [
      {
        ...
        onClick: () => {
          ...
          window.api.file.save(fileName, message.content).then()
        }
      },
  ...
  const onUseful = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation()
      editMessage(message.id, {useful: !message.useful}).then()
    },
    [message, editMessage]
  )
