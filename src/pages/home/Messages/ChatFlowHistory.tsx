...
import {selectTopicMessages} from '@/store/messages'
...
const CustomNode: FC<{ data: any }> = ({data}) => {
  ...
  const handleNodeClick = () => {
    if (data.messageId) {
      ...
      setTimeout(() => {
        EventEmitter.emit(EVENT_NAMES.LOCATE_MESSAGE + ':' + data.messageId).then()
      }, 250)
    }
  }
...
const ChatFlowHistory: FC<ChatFlowHistoryProps> = ({conversationId}) => {
  ...
  const messages = useSelector(
    memoize((state: RootState) => selectTopicMessages({state, topicId: topicId || ''})),
    (prev, next) => {
