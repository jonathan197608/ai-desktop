...
import { LOAD_MORE_COUNT } from '@/config/constant'
import db from '@/databases'
import { useAssistant } from '@/hooks/useAssistant'
import { useMessageOperations, useTopicLoading, useTopicMessages } from '@/hooks/useMessageOperations'
import { useSettings } from '@/hooks/useSettings'
import { useShortcut } from '@/hooks/useShortcuts'
import { autoRenameTopic, getTopic } from '@/hooks/useTopic'
import { getDefaultTopic } from '@/services/AssistantService'
import { EVENT_NAMES, EventEmitter } from '@/services/EventService'
import { getContextCount, getGroupedMessages, getUserMessage } from '@/services/MessagesService'
import { estimateHistoryTokens } from '@/services/TokenService'
...
const Messages: React.FC<MessagesProps> = ({ assistant, topic, setActiveTopic }) => {
  ...
  useEffect(() => {
    const unsubscribes = [
      ...
      EventEmitter.on(EVENT_NAMES.EXPORT_TOPIC_IMAGE, async () => {
        ...
        if (imageData) {
          window.api.file.save(removeSpecialCharactersForFileName(topic.name), imageData)
        }
      }),
  ...
  useShortcut('copy_last_message', () => {
    ...
    if (lastMessage) {
      ...
      window.message.success(t('message.copy.success')).then()
    }
  })
